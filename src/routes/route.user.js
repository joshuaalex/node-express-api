...
module.exports = () => {
    ...
    api.post('/', authorizer(['admin', 'registered']), async (req, res) => {
        try {
            ...
            const smsresponse = await SmsServiceDao.sendSms(text, "07066031881", "LexClass");
    ...
    api.delete('/:id', authorizer(['admin']), async (req, res) => {
        ...
        if (id) {
            try {
                ...
                res.status(200).json({ status: 'success', payload: null, message: 'User Deleted Successfully!'+id });
            } catch (err) {
                res.status(500).json({ status: 'failed', payload: null, message: err });
            }
        } else {
